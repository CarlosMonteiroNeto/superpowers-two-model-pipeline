...
| `brief-scaffold WORKSPACE TASK` (two-model) | Scaffold the task brief mechanically from the plan (statement, acceptance, spec_refs, reason-declaration and machine-readable RED instructions). The RED order also scopes the operador to the scoped `rtk-run` runner (compressed output, evidence where the gate reads it) and forbids the full suite, which `coder-gate` owns (Change 2 + T1, ADR-0016). Rejects test-like `touches` (test files are defined as changed minus `touches`, so a test path inside `touches` can never prove RED). No LLM | exit 0 wrote; 1 plan unreadable; 2 usage/missing fields/test-like touches |
...
| `rtk-run [--ws WS --task N] <red\|test\|analyze> [ARGS...]` (flutter) | The Agente operador's SCOPED runner (T1, ADR-0016). It accepts only three modes and delegates to `cmd`, so the operador's test output is RTK-compressed while the FULL output lands on disk: `red` runs `flutter test --machine` and writes `WS/task-N-red.txt` (the exact path `red-form-check` reads), `test` runs `flutter test`, `analyze` runs `flutter analyze`. Because the mode is a closed set, the operador's bash allowlist permits exactly this runner — matched by an install-independent two-sided wildcard (`*flutter-app-pipeline/scripts/rtk-run*`, permission `*` matches any character), so it works from any checkout and however the shell wraps the call (`& "<path>"`, `bash "<path>"`, or bare) — without granting arbitrary execution (which a generic `cmd` entry would). The coder definition also ships platform-generic READ-ONLY allowances (Windows cmdlets `Get-ChildItem`/`Get-Content`/… and POSIX `ls`/`cat`/`grep`/…), because a per-machine override is what let install-specific paths leak into the mirror; the LIVE definition and the mirror are kept identical. The brief mandates it and forbids the full suite | the wrapped command's exit code; 2 usage |
