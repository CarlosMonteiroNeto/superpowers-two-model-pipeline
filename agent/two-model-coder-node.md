---
description: Operational tier of the two-model pipeline (GPT-6 Luna, max reasoning; Node/JavaScript). Owns the RED/GREEN loop - authors RED tests, runs them to save the runner's machine-readable evidence, declares and confirms the reason before implementing; may run test/analyze/format, never git.
mode: all
