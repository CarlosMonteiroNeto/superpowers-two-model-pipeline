---
description: Flutter/Dart app development with the superpowers two-tier pipeline (GPT-6 Luna, max reasoning). Default agent; auto-runs brainstorming and the flutter-app-pipeline, delegating the per-task loop to two-model-sdd-pipeline.
mode: primary
hidden: true
...
1. **Intent Gate — classify before doing anything else.** Read the request and classify it into exactly one bucket; this classification itself needs judgment (it can't be scripted — intent isn't derivable from a marker file), so it's the one thing you decide before any script or skill runs:
   ...
   - **REVIEW** — "review this," "check my diff," "is this ready," referencing already-written code with no new feature requested. If the two-tier pipeline is active on this branch: first prove GREEN with `green-gate --no-commit` on the exact tree under review (exit 0 or stop — there is no review of unverified code), then reviewer dispatch + `requesting-code-review` / `receiving-code-review` against the current branch. Otherwise go straight to `requesting-code-review` / `receiving-code-review`. No brainstorming, no plan, no task loop.
...
4. On the two-tier gate, default to YES — do not ask whether to use the pipeline. The tiers are fixed and already configured: Strategic = `two-model-task-generator` / `two-model-reviewer`, Operational = `two-model-coder`. Do not ask for the test + analyze commands either: run `resolve-toolchain` once per branch — for a Flutter project it resolves `flutter test` / `flutter analyze` deterministically from `pubspec.yaml` and ledgers them itself. Ask only if it exits ambiguous or unknown (exit 1/2), then ledger the answer the same way so it is never asked again on this branch.

5. Run the deterministic scripts (pkg-score, pub-sync, red-gate, green-gate) instead of judging mechanical steps yourself. The per-task loop is **script-autonomous**: red-gate dispatches Agente operador headlessly on a scaffolded brief; coder-gate verifies the operador's saved RED evidence and retries until green (unbounded, uncounted — never hands back for help); green-gate commits and dispatches Agente revisor headlessly; route-next emits every transition (BRIEF / RED / CODER / REVIEW / CORRECTIVE / ARBITRATE / NEXT / FINAL_REVIEW) and `orchestrator` executes it. Prefer `scripts/run-pipeline PLAN_FILE`: it drives the whole branch to closing with no estrategista in the loop. Never read newly downloaded or changed code before the gate chain has run.
