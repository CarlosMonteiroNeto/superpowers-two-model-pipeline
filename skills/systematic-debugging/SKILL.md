...
- **Compress before you read.** Route any log, stack trace, or diagnostic dump through `scripts/cmd --full-file FILE -- CMD...` (RTK minification) before it enters context — the 60-90% token savings matters most exactly here, where debugging tends to pull in the largest raw artifacts. The full output still lands in the file, so nothing a verdict depends on is lost; `cmd` passes through unchanged when RTK has no filter for the format.
