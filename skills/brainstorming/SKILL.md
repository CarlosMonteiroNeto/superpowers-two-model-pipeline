...
- Fact-finding questions (objective, constraints only the developer knows) stay fully open-ended — no preset options. Preset choices anchor the developer to your assumptions instead of surfacing theirs
...
- A resolved decision becomes an ADR (`docs/superpowers/adr/NNNN-title.md`) only when it passes all three gates simultaneously:
  ...
  3. **Rejected alternative** — a real alternative was considered and rejected for a stated reason.
  Most sessions produce zero ADRs. That is the process working, not failing.

**Exploring approaches:**

- Propose exactly 3 different approaches with trade-offs, plus an explicit free-form custom-answer option ("none of these, here's what I actually want")
- This fixed format applies at the design-choice step only — fact-finding questions keep no presets
