...
- **The knowledge graph is rebuilt per worktree root.** `graphify-update [ROOT]` / `graphify-regen [ROOT]` take the worktree root explicitly and run just before each task's commit (never mid-edit) — don't assume a single global graph when working across multiple worktrees at once.
