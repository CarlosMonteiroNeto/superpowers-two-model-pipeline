...
- **Work state is per worktree root.** `pipeline-workspace` resolves the task-state directory from the worktree root — don't assume a single global workspace when working across multiple worktrees at once.
