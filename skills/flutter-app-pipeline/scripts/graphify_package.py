...
then runs `graphify` on it so the LLM queries the interface instead of reading
raw source.

Usage:
  graphify-package PACKAGE [--config PACKAGE_CONFIG] [--out DIR] [--root DIR]
...
def main(argv=None):
    ...
    subprocess.run(
        [GRAPHIFY_BIN, pkg_dir, "--out", args.out],
        check=True,
    )
