...
- **Optional Site 5 fusion is planning-only.** After a complete draft is valid, `plan-fusion propose DRAFT --workspace DIR --report REPORT` records shadow Jev recommendations. A strategist may explicitly select qualifying pairs in a separate selection file, then run `plan-fusion apply ...` to write a distinct pre-runtime plan. The tool never applies recommendations automatically, never changes a runtime workspace plan, and shadow agreement is not evidence of an executed fusion outcome.
