...
| Implementer subagent | **Agente operador**, `two-model-coder` (Operational tier, write-only, authors RED tests, never runs commands, unbounded retries until green, resumable within a task via `--continue`) |
...
- **RED tests move to Agente operador authorship.** The single biggest behavioral change from the generic skill above: Agente diretor writes the task's acceptance + `expected_red` into the plan; the operador authors the RED tests from the scaffolded brief, and `coder-gate`'s RED-proof verifies they fail pre-implementation *before* any commit. The implementer only ever sees the brief plus its own RED-proof gate — see the updated `implementer-prompt.md` and `two-model-sdd-pipeline/coder-prompt.md`'s "NEVER weaken a test" rule.
