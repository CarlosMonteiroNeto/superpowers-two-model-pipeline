...
|---|---|
| Orchestrating session | **Script CEO** (deterministic; gates, routing, dispatch, commits, ledger — never implements or reviews) |
| Planning / task breakdown | **Agente estratégico** (spec + complete `plan.json` with acceptance/spec_refs, then DONE); **Agente diretor** is punctual (correctives, arbitrate, closing) — no expansion and no `expected_red` (ADR-0008) |
| Implementer subagent | **Agente operador**, `two-model-coder` (Operational tier, owns the RED/GREEN loop: authors RED tests, runs them to confirm the expected failure, then implements; may run test/analyze/format, never git; unbounded retries until green, resumable within a task via `--continue`) |
| Reviewer subagent | **Agente revisor**, `two-model-reviewer` (Strategic tier, structured JSON verdict only: APPROVED / SEND_BACK / ESCALATE; judges test-vs-acceptance fit; never runs test/analyze itself) |
| Ad-hoc "strategic coder" escalation | **Removed.** Escalation is Agente diretor's `ARBITRATE` step — there is no third coder tier. |
...
- **RED tests move to Agente operador authorship.** The single biggest behavioral change from the generic skill above: the complete `plan.json` carries the task's acceptance + spec_refs (no `expected_red`); the operador authors the RED tests from the scaffolded brief AND runs them itself, saving the runner's machine-readable output, and `coder-gate` validates that saved RED **form** with `red-form-check` before any commit. The implementer only ever sees the brief plus its own RED/GREEN loop — see `two-model-sdd-pipeline/coder-prompt.md`'s "NEVER weaken a test" rule.
