...
| Planning / task breakdown | **Agente estratégico** (plan shell + spec, then DONE) + **Agente diretor** (task expansion with acceptance + expected_red, correctives, closing) |
...
- **RED tests move to Agente operador authorship.** The single biggest behavioral change from the generic skill above: Agente diretor writes the task's acceptance + `expected_red` into the plan; the operador authors the RED tests from the scaffolded brief AND runs them itself to confirm the expected failure (saving the output), and `coder-gate` verifies that evidence before any commit. The implementer only ever sees the brief plus its own RED/GREEN loop — see the updated `implementer-prompt.md` and `two-model-sdd-pipeline/coder-prompt.md`'s "NEVER weaken a test" rule.
